...
（N=20 も同様に null）。符号は n=5 時点と同じく二力の予測の向き（GP 正・predation 負）
だが、n=20 でも revisable 世界の分散が支配して有意に届かない。確定文:
「認定 regime 上の batch 変調は n=20 でも検出されない」。

**最終予算（並行書き込み incident 後の artifacts 再構成値、0002a 追記参照）**:
