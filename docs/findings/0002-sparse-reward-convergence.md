...
- **cont の committed vs revisable 差の検定（per-seed 決定論再計算、
  `results/bcs_cont_seeds.csv`）**: per-seed markup は committed
  {−0.03, −0.00, 0.14, 0.18, 0.32} vs revisable {1.46, 1.67, 1.78, 2.10, 2.50} で
  **完全分離**（max committed 0.32 < min revisable 1.46）。主検定 = 完全分離の
  **exact Mann-Whitney 両側 p = 2/252 ≈ 0.008**（分布仮定ゼロ）、参考 Welch t ≈ 9.2。
  同一 master seed は条件横断で同一の子ストリームを spawn する（D-B12）ため
  実際には**対応あり構造**であり、5 ペア全てで差が正（+1.46〜+2.18、
  sign test exact 両側 p = 0.0625）。記述的事実（完全分離）＋仮定最小の正確検定
  ＋対応あり併記、の順で仮定依存を最小化する（旧記述「非対の比較で約 9SE」は
  この確定記述で置換——CRN 構造の見落としを訂正）。
- **二レンズの読み（writeup に固定する構成）**: 認定ゲートのレンズでは現実密度で
  何も見えない（認定ゼロ）が、**分布のレンズでは同じ較正点の revisable 世界で
  markup が競争水準の 1.5–1.9 倍に滞留している**。つまり「収束した政策としての
...
（追加検証の候補としてのみ記録。やるなら新規事前登録で）。

**規則 4（Δ 帰属の n=20 再判定）**: 実行中（`results/attribution_seeds20.*` に追記予定）。

P2 主張 (ii) の最終形（降格枝、§2.5 の非対称どおり）:
「認定可能な regime は (ν=30, lr=0.15) 近傍の revisable 世界に** n=5 プール水準で**
存在するが、n=20 の厳格プール基準（収束全数）は生き残らない（18/20）。
supra-competitive 帯域への滞留（markup 50–74、SE 2.4–8.6）は seed・アルゴ・HP
横断で再現する」——認定ゲートのレンズと分布のレンズの乖離は、ここでも US4 と
同じ構造で現れている。
