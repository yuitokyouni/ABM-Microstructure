...
- **促進方向**：tacit collusion は監視の良さ（行動の可観測性）と反復性によって維持される（**Green & Porter 1984** 系）。batch auction は離散・透明な clearing であり、逸脱の検知と懲罰を容易にしうる。
- **破壊方向（当初の Bertrand 借用は誤り・訂正済）**：「uniform-price batch では微小 undercut で batch 全取り → 不安定化」は **Bertrand 直観の誤った密輸**だった。uniform-price clearing では marginal quote が約定全量の受取価格に効くため、undercut は自分の受取価格を不利に動かす（**demand-reduction 誘因**）。よって破壊方向の脚は見かけより弱い。

**帰結（confirmation risk として明示）**：破壊方向が弱い以上、理論的 prior は「batch は collusion を促進」側に傾く。期待結果＝trade-off であり、sim が傾いた prior をただ追認する危険が上がる（A3 の artifact 懸念と同型）。よって prior の傾きを自覚した上で、null（§6）と妥当性検査（A3／§3.4 の gate）を特に固く回す。どちらが勝つかは解析では出ず、**地図上のどこで何が起きるかが結果である。**
