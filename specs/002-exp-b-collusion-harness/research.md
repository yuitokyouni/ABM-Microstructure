...
- **Decision**: **本 feature の総予算 = 3×10⁹ 学習期**（runner が累計をカウントし、超過する run を起動拒否）。配分：
  ...
  - **実装注記（2026-06-11, crash 精算ルール）**: coarse 本番 run（360 runs 並列）が ~70/72 セル完了後に worker プロセス喪失（BrokenProcessPool）で落ち、CSV 未生成のまま charge ≈7.4×10⁸ 期が ledger に残った。対応を2点固定する。(1) **runner の crash 耐性**: セル完了ごとに CSV へ追記し、再実行は既存 out の完了セルを skip する（resume。cell id が一意でない robustness 変種列では起動拒否）。worker 喪失は当該 job のみ落として続行し charge は残す。(2) **精算ルール**: 成果物（CSV）として保持されなかった run の charge は `BudgetLedger.reconcile` で返金できるが、必ず ledger の `reconciliations` に期数と理由を恒久記録する（黙った減額は不可）。根拠: 予算の目的は B1＝設計空間の漸進的拡大の遮断であり、D-B12 の決定論により同一の事前登録 grid の再実行は bit 同一の再計算＝追加サンプリングではない。保持成果物が背書きする期数（pilot2.csv の 16,080,800 期——finding 0002 の pilot 表の出典）は返金しない。本件の精算: coarse 755,317,603 − 16,080,800 = **739,236,803 期を返金**（crash log は results/coarse.crash-20260611.log として保全）。
